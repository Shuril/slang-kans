        ...
        if use_gpu:
            try:
                ...
                if mgr.is_gpu_available():
                    dev = mgr.device
                    buf_s = dev.create_buffer(data=np.ascontiguousarray(self.scales.reshape(-1)), usage=slangpy.BufferUsage.shader_resource)
                    buf_b = dev.create_buffer(data=np.ascontiguousarray(self.biases.reshape(-1)), usage=slangpy.BufferUsage.shader_resource)
                    buf_out = dev.create_buffer(size=total * 4, usage=slangpy.BufferUsage.shader_resource | slangpy.BufferUsage.unordered_access)
                    if self.bits == 8:
                        k8 = mgr.get_or_compile_kernel("quantize", "dequantize_int8")
                        buf_p = dev.create_buffer(data=np.ascontiguousarray(self.packed_weight.reshape(-1).astype(np.uint32)), usage=slangpy.BufferUsage.shader_resource)
                        k8.dispatch(
                            thread_count=[total, 1, 1],
                            packed_weight=buf_p,
                            scales=buf_s,
                            biases=buf_b,
                            output=buf_out,
                            num_groups=out_f * num_groups,
                            group_size=g_size
                        )
                        dev.wait_for_idle()
                        w_flat = buf_out.to_numpy().view(np.float32)[:total].reshape(out_f, -1)
                        if self.pad_len > 0:
                            w_flat = w_flat[:, :self.in_features]
                        return w_flat.reshape(self.orig_shape)
                    elif self.bits == 4:
                        k4 = mgr.get_or_compile_kernel("quantize", "dequantize_int4")
                        buf_p = dev.create_buffer(data=np.ascontiguousarray(self.packed_weight.reshape(-1).astype(np.uint32)), usage=slangpy.BufferUsage.shader_resource)
                        k4.dispatch(
                            thread_count=[total, 1, 1],
                            packed_weight=buf_p,
...
def get_model_size(model: Any) -> Dict[str, Any]:
    ...
    def inspect_obj(obj):
        ...
        elif isinstance(obj, TernaryQuantizedWeight):
            total_bytes += obj.w_packed.nbytes + obj.scales.nbytes
            total_params += int(np.prod(obj.shape))
        elif isinstance(obj, Int2QuantizedWeight):
            total_bytes += obj.w_packed.nbytes + obj.scales.nbytes + obj.codebook.nbytes
            total_params += int(np.prod(obj.shape))
...
class TernaryQuantizedWeight:
    ...
    def __init__(self, weight: np.ndarray, group_size: int = 32):
        ...
        K_dim = weight.size // D_out

        w = np.ascontiguousarray(weight.reshape(D_out, K_dim), dtype=np.float32)
    ...
    def dequantize(self, use_gpu: bool = True) -> np.ndarray:
        D_out = self.shape[0]
        K_dim = int(np.prod(self.shape[1:]))
        ...
        if use_gpu:
            try:
                ...
                if mgr.is_gpu_available():
                    dev = mgr.device
                    k_ter = mgr.get_or_compile_kernel("quantize", "dequantize_ternary_fast")
                    buf_p = dev.create_buffer(data=np.ascontiguousarray(self.w_packed), usage=slangpy.BufferUsage.shader_resource)
                    buf_s = dev.create_buffer(data=np.ascontiguousarray(self.scales), usage=slangpy.BufferUsage.shader_resource)
                    buf_out = dev.create_buffer(size=D_out * K_dim * 4, usage=slangpy.BufferUsage.shader_resource | slangpy.BufferUsage.unordered_access)
                    k_ter.dispatch(
                        thread_count=[K_dim, D_out, 1],
                        packed_weight=buf_p,
                        scales=buf_s,
                        output=buf_out,
                        D_out=D_out,
                    ...
                    dev.wait_for_idle()
                    out = buf_out.to_numpy().view(np.float32)[:D_out * K_dim].reshape(self.shape)
                    return out
            except Exception:
                pass
        ...
        for j in range(D_out):
            ...
            for k in range(K_dim):
                word_idx = k // 16
                shift = (k % 16) * 2
                code = (w_row[word_idx] >> shift) & 0x03
                w_val = -1.0 if code == 0 else (1.0 if code == 2 else 0.0)
                g = k // self.group_size
                sc = self.scales[j, g]
                out[j, k] = w_val * sc
    ...
    def dequantize(self, use_gpu: bool = True) -> np.ndarray:
        ...
        if use_gpu:
            try:
                ...
                if mgr.is_gpu_available():
                    dev = mgr.device
                    k_int2 = mgr.get_or_compile_kernel("quantize", "dequantize_int2_fast")
                    buf_p = dev.create_buffer(data=np.ascontiguousarray(self.w_packed), usage=slangpy.BufferUsage.shader_resource)
                    buf_s = dev.create_buffer(data=np.ascontiguousarray(self.scales), usage=slangpy.BufferUsage.shader_resource)
                    buf_c = dev.create_buffer(data=np.ascontiguousarray(self.codebook), usage=slangpy.BufferUsage.shader_resource)
                    buf_out = dev.create_buffer(size=D_out * K_dim * 4, usage=slangpy.BufferUsage.shader_resource | slangpy.BufferUsage.unordered_access)
                    k_int2.dispatch(
                        thread_count=[K_dim, D_out, 1],
                        packed_weight=buf_p,
        ...
        for j in range(D_out):
            ...
            for k in range(K_dim):
                word_idx = k // 16
                shift = (k % 16) * 2
                c_idx = (w_row[word_idx] >> shift) & 0x03
                g = k // self.group_size
                sc = self.scales[j, g]
...
def to_ternary(model: Any, group_size: int = 32) -> Any:
    """Quantizes all weight matrices of a slang-KAN layer or network to 1.58-bit Ternary in-place."""
    if hasattr(model, "layers") and isinstance(model.layers, list):
        for layer in model.layers:
            to_ternary(layer, group_size=group_size)
        return model

    if hasattr(model, "weights") and isinstance(model.weights, np.ndarray):
        qw = TernaryQuantizedWeight(model.weights, group_size=group_size)
        setattr(model, "_weights_ternary", qw)
        model.weights = qw.dequantize()
        if hasattr(model, "_buf_w"):
            model._buf_w = None
        if hasattr(model, "_buf_w_gemm"):
            model._buf_w_gemm = None
    return model


def to_int2(model: Any, group_size: int = 32, codebook: Any = None) -> Any:
    """Quantizes all weight matrices of a slang-KAN layer or network to 2-bit Codebook in-place."""
    if hasattr(model, "layers") and isinstance(model.layers, list):
        for layer in model.layers:
    ...
    if hasattr(model, "weights") and isinstance(model.weights, np.ndarray):
        qw = Int2QuantizedWeight(model.weights, group_size=group_size, codebook=codebook)
        setattr(model, "_weights_int2", qw)
        model.weights = qw.dequantize()
        if hasattr(model, "_buf_w"):
            model._buf_w = None
        if hasattr(model, "_buf_w_gemm"):
