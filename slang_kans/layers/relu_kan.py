...
class ReLUKANLinear(SlangKANLayerBase):
    ...
    def backward(self, dY: np.ndarray) -> np.ndarray:
        """
        Analytical backward pass for piecewise linear tent ReLUKAN.
        ...
        raw_diff = x_arr[..., None] - self.centers[None, None, :]
        abs_diff = np.abs(raw_diff) * self.inv_h
        mask = (abs_diff < 1.0).astype(np.float32)
        bases = np.maximum(0.0, 1.0 - abs_diff)

        dW = np.einsum("bic,bj->ijc", bases, dY_arr)
        if self.grad_weights is not None:
        ...
        # Derivative of max(0, 1 - |x-c|/h): -sign(x-c) * inv_h when |x-c| < h
        sign_diff = np.sign(raw_diff)
        dbases_dx = -sign_diff * self.inv_h * mask

        dL_dbases = np.einsum("bj,ijc->bic", dY_arr, self.weights)
        dX = np.sum(dL_dbases * dbases_dx, axis=-1)
