...
class FourierKANLinear(SlangKANLayerBase):
    ...
    def _sync_gpu_weights(self):
        ...
        if self._kernel is None:
            ...
            self._kernel_basis = self.mgr.get_or_compile_kernel("fourier", "eval_fourier_basis")
    ...
    def _benchmark_gpu(self, x: np.ndarray, warmup: int = 10, iters: int = 50) -> float:
        ...
        for _ in range(warmup):
            if B >= 64:
                self._kernel_basis.dispatch(thread_count=[B, D_in, 1], phi=buf_phi, input=buf_x, batch_size=B, in_features=D_in, num_frequencies=self.num_frequencies)
                ...
                self._kernel.dispatch(thread_count=[B, (D_out + 3) // 4, 1], output=buf_out, input=buf_x, weights=self._buf_w, bias=self._buf_b, batch_size=B, in_features=D_in, out_features=D_out, num_frequencies=self.num_frequencies)
        dev.wait_for_idle()
